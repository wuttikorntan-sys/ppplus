          ...
          <div className="grid lg:grid-cols-2 gap-12">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="space-y-6">
                <div className="bg-white rounded-2xl p-6 shadow-sm flex items-start gap-4">
                  <div className="bg-[#1E3A5F]/10 p-3 rounded-xl"><MapPin className="w-6 h-6 text-[#1E3A5F]" /></div>
                  <div>
                    <h3 className="font-semibold text-[#1E293B] mb-1">{lang === 'th' ? 'ที่อยู่' : 'Address'}</h3>
                    <p className="text-[#64748B] text-sm">{v('location.address') || t('info.address')}</p>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-6 shadow-sm flex items-start gap-4">
                  <div className="bg-[#2EC4B6]/10 p-3 rounded-xl"><Phone className="w-6 h-6 text-[#2EC4B6]" /></div>
                  <div>
                    <h3 className="font-semibold text-[#1E293B] mb-1">{lang === 'th' ? 'โทรศัพท์' : 'Phone'}</h3>
                    <p className="text-[#64748B] text-sm">{v('location.phone') || t('info.phone')}</p>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-6 shadow-sm flex items-start gap-4">
                  <div className="bg-[#2EC4B6]/10 p-3 rounded-xl"><Mail className="w-6 h-6 text-[#2EC4B6]" /></div>
                  <div>
                    <h3 className="font-semibold text-[#1E293B] mb-1">{lang === 'th' ? 'อีเมล' : 'Email'}</h3>
                    <p className="text-[#64748B] text-sm">{v('location.email') || t('info.email')}</p>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-6 shadow-sm flex items-start gap-4">
                  <div className="bg-[#1E3A5F]/10 p-3 rounded-xl"><MessageCircle className="w-6 h-6 text-[#1E3A5F]" /></div>
                  <div>
                    <h3 className="font-semibold text-[#1E293B] mb-1">LINE</h3>
                    <p className="text-[#64748B] text-sm">{v('location.line') || t('info.line')}</p>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-6 shadow-sm flex items-start gap-4">
                  <div className="bg-[#1E3A5F]/10 p-3 rounded-xl"><Clock className="w-6 h-6 text-[#1E3A5F]" /></div>
                  <div>
                    <h3 className="font-semibold text-[#1E293B] mb-1">{v('location.hours_title') || t('info.hours_title')}</h3>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-6">
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 md:p-8 shadow-sm space-y-5">
                <h3 className="text-lg font-semibold text-[#1E293B]" style={{ fontFamily: 'var(--font-heading)' }}>
                ...
                {sent ? (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8 space-y-3">
                    ...
                    <button type="button" onClick={() => setSent(false)} className="mt-2 text-[#1E3A5F] text-sm font-medium hover:underline">
                  ...
                  <>
                    <div>
                      ...
                      <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#2EC4B6]/20 focus:border-[#2EC4B6] outline-none transition text-sm" />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#2EC4B6]/20 focus:border-[#2EC4B6] outline-none transition text-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">{lang === 'th' ? 'เบอร์โทร' : 'Phone'}</label>
                        <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#2EC4B6]/20 focus:border-[#2EC4B6] outline-none transition text-sm" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('form.message')} <span className="text-red-500">*</span></label>
                      <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} required rows={5} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#2EC4B6]/20 focus:border-[#2EC4B6] outline-none transition resize-none text-sm" />
                    </div>
                    <button type="submit" disabled={sending} className="w-full py-3.5 bg-[#1E3A5F] text-white rounded-xl font-semibold hover:bg-[#2a4d75] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
