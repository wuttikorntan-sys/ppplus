        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              {/* Global Meta Tags */}
              <div>
                <h3 className="text-sm font-semibold text-gray-700 mb-3">{th ? 'Meta Tags ทั่วไป' : 'Global Meta Tags'}</h3>
                <div className="space-y-3">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-700">{th ? 'หน้าใน Sitemap' : 'Sitemap Pages'}</h3>
                ...
                <p className="text-xs text-gray-400 mt-2">{th ? 'Path ต้องขึ้นต้นด้วย / (หน้าแรกใช้ค่าว่าง) — URL สำหรับทุกภาษาสร้างอัตโนมัติ' : 'Path must start with / (empty for homepage) — URLs for all locales are generated automatically'}</p>
              </div>

              {/* Robots.txt Disallow */}
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-gray-700">{th ? 'Robots.txt - เส้นทางที่ไม่อนุญาต' : 'Robots.txt - Disallowed Paths'}</h3>
                ...
                <p className="text-xs text-gray-400 mt-2">{th ? 'เส้นทางที่ bot จะไม่สามารถเข้าถึงได้ (เช่น /api/ /admin/)' : 'Paths that bots cannot access (e.g. /api/ /admin/)'}</p>
              </div>
            </div>
          )}
        </div>
